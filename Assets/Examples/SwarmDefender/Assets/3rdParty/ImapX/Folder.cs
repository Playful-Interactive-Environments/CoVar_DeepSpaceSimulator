...
namespace ImapX
{
    ...
    public class Folder
    {
        ...
        internal long[] SearchMessageIds(string query = "ALL", int count = -1)
        {
            ...
            return count < 0
                ? result.Groups[1].Value.Trim().Split(' ').Select<string, long>(long.Parse).ToArray()
                : result.Groups[1].Value.Trim().Split(' ').OrderBy(_ => _).Take(count).Select<string, long>(long.Parse).ToArray();
        }
